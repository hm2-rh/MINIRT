...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/03/06 18:00:15 by hrhirha           #+#    #+#             */
/*   Updated: 2020/03/11 17:19:30 by hrhirha          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	calc_intensity(t_data *data, t_vec inter_pt, t_vec n)
{
	...
	while (tmp_lt)
	{
		...
		data->ray_lt.start = addvec(data->hit_pt,
							mulvec(.0000001, data->hit_normal));
		data->ray_lt.dir = normalize(subvec(data->light->pos, data->hit_pt));
		tmp = data->ray;
		data->ray = data->ray_lt;
		light_dir = normalize(subvec(data->light->pos, inter_pt));
		data->hit_lt = first_inter(data, &data->hit_pt_lt, &data->hit_normal_lt,
									&data->hit_color_lt);
		intensity_value(data, n, light_dir, &intensity);
		tmp_lt = tmp_lt->next;
		data->ray = tmp;
	}
	return (intensity);
